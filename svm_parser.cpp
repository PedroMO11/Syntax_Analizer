...
const char *Token::token_names[24] = {
    "ID",    "LABEL", "NUM",   "EOL",   "ERR",  "END",   "PUSH", "JMEPEQ",
    "JMPGT", "JMPGE", "JMPLT", "JMPLE", "GOTO", "SKIP",  "POP",  "DUP",
    "SWAP",  "ADD",   "SUB",   "MUL",   "DIV",  "STORE", "LOAD", "PRINT"};
...
void Scanner::incrStartLexema() { first++; }
...
Instruction *Parser::parseInstruction() {
  ...
  if (match(Token::SKIP) || match(Token::POP) || match(Token::DUP) ||
      ...
      match(Token::MUL) || match(Token::DIV) ||
      match(Token::PRINT)) { // mas casos
    tipo = 0;
    ttype = previous->type;
  } else if (match(Token::PUSH) || match(Token::STORE) ||
             match(Token::LOAD)) { // mas casos
    tipo = 1;
    ttype = previous->type;

  } else if (match(Token::JMPEQ) || match(Token::JMPGT) ||
             match(Token::JMPGE) || match(Token::JMPLT) ||
             match(Token::JMPLE) || match(Token::GOTO)) { // mas casos
    tipo = 2;
    ttype = previous->type;
  ...
  if (tipo == 0) {
    ...
    instr = new Instruction(label, Token::tokenToIType(ttype), current->lexema);
    this->advance();
  }
