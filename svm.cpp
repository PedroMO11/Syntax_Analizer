...
string snames[18] = {"push",  "pop",   "dup",  "swap",  "add",   "sub",
                     "mult",  "div",   "goto", "jmpeq", "jmpgt", "jmpge",
                     "jmplt", "jmple", "skip", "store", "load",  "print"};

Instruction::Instruction(string l, IType itype)
    : label(l), type(itype), hasarg(false) {}

Instruction::Instruction(string l, IType itype, int arg)
    : label(l), type(itype), hasarg(true), argint(arg), jmplabel("") {}

Instruction::Instruction(string l, IType itype, string arg)
    : label(l), type(itype), hasarg(true), jmplabel(arg) {}
...
void SVM::execute(Instruction *instr) {
  ...
  if (itype == Instruction::IPOP || itype == Instruction::IDUP ||
    ...
    case (Instruction::IPOP):
      if (opstack.empty())
        perror("Can't pop from an empty stack");
      opstack.pop();
      break;
    case (Instruction::IDUP):
      if (opstack.empty())
        perror("Can't dup from an empty stack");
    ...
    case (Instruction::ISTORE):
      if (opstack.empty())
        perror("Can't store from an empty stack");
    ...
    if (jump)
      ...
      pc++;
  } else if (itype == Instruction::IADD || itype == Instruction::ISUB ||
             itype == Instruction::IMUL || itype == Instruction::IDIV ||
